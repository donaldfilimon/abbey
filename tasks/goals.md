...
Canonical capability ledger: **27 Current · 1 Partial · 8 Proposed · 1 Blocked · 5 Out of scope** ([full evidence](../docs/claims.md)). Source: `src/claims.rs`; schema 1; digest `de3e3e300e4b9ae59d2cc385240017bbbc4ca06077440a8906c6408933b1a582`.
Executable workflow ledger: **26 goals (23 done, 1 in_progress, 1 proposed, 1 blocked) · 117 checked / 20 open todos** (stable goal metadata in `tasks/goals.md`).
...
status: in_progress
implementation-evidence: Phase 1 baseline, Phase 3 executable ledgers, read-only daemon, Phase 4A durable lifecycle, Phase 4B.1 bounded delegated supervision, Phase 4B.2 authenticated protocol-v2 fixed-route run control, and Phase 4B.3 shared CLI/TUI lifecycle reduction are complete
automated-test-evidence: check.sh all feature modes plus claims-sync, rustdoc, RustSec, release, installer, semantic, mesh, inventory, daemon, protocol compatibility, fixed-watermark paging, frontend-neutral reducer fixtures, CLI/TUI slash parity, repeated runtime state-machine suites, and repeated process-group race proof
live-external-evidence: local macOS real-binary protocol-v2 submission idempotency cancellation descendant teardown restart/reopen CLI/slash parity and disclosure proof is green; TUI parity is deterministic slash-process evidence rather than interactive-terminal proof; hosted runners signing DAST paid provider and separate-platform proofs remain distinct
next-action: Phase 4B.4 — idempotent legacy conversation metadata migration into runtime.sqlite on scratch copies, with retained backup and no transcript or semantic-memory import
-->
