...
Executable workflow ledger: **26 goals (23 done, 1 in_progress, 1 proposed, 1 blocked) · 143 checked / 22 open todos** (stable goal metadata in `tasks/goals.md`).
...
status: in_progress
implementation-evidence: Phase 1 baseline, Phase 3 executable ledgers, read-only daemon, Phase 4A durable lifecycle, Phase 4B.1 bounded delegated supervision, Phase 4B.2 authenticated protocol-v2 fixed-route run control, Phase 4B.3 shared CLI/TUI lifecycle reduction, Phase 4B.4 backup-first legacy conversation metadata migration, Phase 4B.5 canonical-first conversation identity saves, Phase 4B.6 canonical scope/all tombstones with edition-scoped compatibility-mirror removal, Phase 4B.7 canonical edition-scoped conversation reads, Train 1.4 bounded daemon handoff and reap-first child teardown, Train 1.5 transactional accelerator installation, Train 1.6 near-cap decomposition, and the merged ABI foundation through 88c02fb550169e4cdb5e1df2bf6d1d13532e0e49 are complete; Abbey pins and compiles the ABI agent host/runtime, model registry/runtime, and worker crates, shares ABI cooperative cancellation, exposes a startup-owned finite-budget ModelProvider execution bridge, and provides fixed-recipe Cursor, Grok, ABI-local, and Foundation Models adapters without changing protocol v1/v2 authority
