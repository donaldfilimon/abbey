...
Canonical capability ledger: **30 Current · 1 Partial · 8 Proposed · 1 Blocked · 5 Out of scope** ([full evidence](../docs/claims.md)). Source: `src/claims.rs`; schema 1; digest `0de544f2bf96e2651c2fd14694df4e6b3801cf5a1d651134588cadf4f245acb6`.
Executable workflow ledger: **26 goals (23 done, 1 in_progress, 1 proposed, 1 blocked) · 119 checked / 20 open todos** (stable goal metadata in `tasks/goals.md`).
...
status: in_progress
implementation-evidence: Phase 1 baseline, Phase 3 executable ledgers, read-only daemon, Phase 4A durable lifecycle, Phase 4B.1 bounded delegated supervision, Phase 4B.2 authenticated protocol-v2 fixed-route run control, Phase 4B.3 shared CLI/TUI lifecycle reduction, Phase 4B.4 backup-first legacy conversation metadata migration, and Phase 4B.5 canonical-first conversation identity saves with edition-scoped compatibility mirrors are complete
automated-test-evidence: check.sh all feature modes plus claims-sync, rustdoc, RustSec, release, installer, semantic, mesh, inventory, daemon, protocol compatibility, fixed-watermark paging, frontend-neutral reducer fixtures, CLI/TUI slash parity, repeated runtime state-machine/process-group race proof, Phase 4B.4 source-boundary/backup/schema-v2/default-WDBX tests, and Phase 4B.5 domain-separated identity, schema-v3 migration, atomic multi-scope revision, marker tamper rejection, locked journal recovery, concurrent save/compaction/capture, unsafe-target, permission/redaction, legacy-clear-parity, and real-CLI default/WDBX tests
live-external-evidence: local macOS real-binary protocol-v2 submission idempotency cancellation descendant teardown restart/reopen CLI/slash parity is green; Phase 4B.4 real scratch abbeyd startup/restart proves unchanged originals and excluded transcript/route/memory/WDBX canaries plus retained owner-only backup and redacted SQLite/output; Phase 4B.5 real scratch CLI default/WDBX save plus post-commit failpoint/reopen proves exact-once mirror/history recovery, opaque SQLite, external-override recovery, owner-only artifacts, and unsafe-parent rejection; TUI parity is deterministic slash-process evidence rather than interactive-terminal proof; hosted runners signing DAST paid provider and separate-platform proofs remain distinct
next-action: Phase 4B.6 — cut clear_chat scope/all mutations over to canonical runtime.sqlite tombstones with the same owner-only journal, idempotent edition-scoped mirror removal, and crash/reopen proof; do not delete transcripts or memory and add no backend/title/run inference, protocol/UI/MCP, or Windows authority
-->
...
- **Completed 2026-08-09 (Phase 4B.5 canonical conversation identity saves):** new Unix
  ...
  real-process proof keeps raw identity/cwd out of SQLite and output. Reads and clear stay
  legacy-mirror behavior; no transcript/memory/backend/title/run inference, protocol/UI/
  MCP surface, or Windows runtime authority is claimed.
...
- **Shipped 2026-08-08 (Phase 9 slice 1 — read-only stdio MCP server):**
  ...
  timeout, redaction, and the no-execution-tool assertion. **Phase 9 is not
  closed:** there is no HTTP transport, so HTTPS, OAuth 2.1/PKCE, resource
  indicators, token-audience binding, rate limiting, hostile-origin, authn, and
  slow-consumer evidence do not exist; `resources`/`prompts`/`tasks`, the
  stateless 2026-07-28 lifecycle, concurrency limits, and cooperative
