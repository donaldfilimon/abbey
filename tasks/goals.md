...
- Docs finished 2026-08-08: corrected three long-standing `fs2`→`fs4` errors and a
  ...
  established.
- Extended 2026-08-19 with a direct Claude Code backend: selection/resolution,
  isolated argv/model mapping, and native session mint/resume are source- and
  unit-tested. No live Claude account, plan, catalog, or response-quality evidence
  was recorded, so those claims remain outside the Current wiring boundary.
...
domain-separated record digests, and keep query-bound fixed snapshots stable across
writes. Payload, provenance, source metadata, raw IDs, backend selection, MCP, semantic
vector search, CLI/desktop presentation, Windows, and remote authority remain outside
this review unit.
