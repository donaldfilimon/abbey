...
Canonical capability ledger: **28 Current · 1 Partial · 8 Proposed · 1 Blocked · 5 Out of scope** ([full evidence](../docs/claims.md)). Source: `src/claims.rs`; schema 1; digest `76785d3106931643981fcfddddae9fd7c0565beeefaf4993fe9af8f98dfeef0e`.
...
status: in_progress
implementation-evidence: Phase 1 baseline, Phase 3 executable ledgers, read-only daemon, Phase 4A durable lifecycle, Phase 4B.1 bounded delegated supervision, and Phase 4B.2 authenticated protocol-v2 fixed-route run control are complete
automated-test-evidence: check.sh both modes plus claims-sync, rustdoc, RustSec, release, installer, semantic, mesh, inventory, daemon, protocol compatibility, fixed-watermark paging, repeated runtime state-machine suites, and repeated process-group race proof
live-external-evidence: local macOS real-binary protocol-v2 submission idempotency cancellation descendant teardown restart/reopen and disclosure proof is green; hosted runners signing DAST paid provider and separate-platform proofs remain distinct
next-action: Phase 4B.3 — shared CLI/TUI lifecycle reducer and frontend-neutral run fixtures over protocol v2
-->
...
- **Completed 2026-08-08 (Phase 4B.2 authenticated bounded run control):** protocol v2
  ...
  v1 remains compatible; CLI/TUI lifecycle parity, live subscriptions, arbitrary tools,
  daemon memory, automations, approvals, Windows transport, and provider-neutral model
  ownership remain later evidence slices.
