...
- [ ] **Blocked external proof:** the 2026-08-09 `startup_failure` / zero-job
      scheduling symptom is historical and no longer current. Runs `33069897239`,
      `33067560632`, and `33065344081` all conclude `success`,
      ...
      former account-level billing-or-quota hypothesis is **no longer supported by evidence** —
...
## Program 3 guild intelligence

- [x] **C1 closed synthetic replay foundation:** accept only explicitly synthetic,
      already-loaded bounded metadata; validate role, channel, thread, bot, and overwrite
      references; normalize five digest-backed twin views; derive metadata-only findings;
      and produce three explicit alternatives with distinct non-executable desired-state
      and rollback previews. A production-gate scanner rejects network, process,
      filesystem, durable-store, runtime-tool, content, approval, and effect surfaces.
- [x] **C2 closed synthetic replay closeout:** prove deterministic and distinct owner and
      administrator fixture replays; exact reference, object, and overwrite limits;
      explicit plan preconditions and postconditions with digest binding; permission
...
- [ ] **Authorized non-synthetic recording capture:** define a separately reviewed
      ...
      redaction, and revocation. Do not reuse the synthetic fixture authority assertion as
      live authorization.
- [ ] **Live read-only validation:** after capture authority exists, record separate
      consented evidence for a real guild observation and compare a closed replay without
      adding message content, member enumeration, persistence, or effects.
- [ ] **Desired-state execution:** remains a separate effect program requiring approvals,
      optimistic concurrency, exact before-state checks, bounded compensation, audit, and
      rollback proof. The closed synthetic plans are data only and grant no execution
      authority.
...
- [ ] **Phase 2 — Self-hosted runners replace the broken hosted-CI assumption:**
  ...
  - Still externally blocked, but narrowed by exact-head macOS success: the Linux ARM64
    runner is not provisioned, the hosted fork job has not executed against an actual
    foreign fork, and Windows runtime evidence stays open.
    ...
    variable/runner, and obtain an actually executed successful Linux Rust workflow run.
...
- [x] **Phase 3 — Claims and ledgers as executable specifications:**
  ...
  - Historical 2026-08-08 claims drift repairs: lexical similarity and opt-in semantic
    memory = separate Current capabilities; learned semantic ranking no longer OOS;
    cursor was then the default provider, not the only backend; fm/abi alternatives;
...
- [ ] **Phase 4 — Shared Abbey application core + durable `abbeyd` daemon:**
  ...
  - **Supervisor teardown flakes diagnosed and fixed 2026-08-09 (`71f2903`):**
    ...
    **After: 0/15** full-suite iterations failed. **Train 1.4 follow-up
    ...
    while 50 isolated repetitions passed. Reader collection therefore got its own doubled
    scheduling window, capped by the existing five-second teardown maximum. **2026-08-19
    correction:** the original 30 ms delayed-reader fixture was not deterministic: its
    worker sleeps began only when the OS scheduled each worker, while the collector's 40 ms
    deadline began immediately. The personal-edition full suite and a 200-run, 32-way stress
    reproduction exposed that race. The test now pins the extracted production window
    calculation directly instead of asserting scheduler timing. Process-group deadlines
    remain unchanged and a genuine pipe leak still fails within a fixed bound.
...
- [ ] **Phase 5 — Model and tool runtime ownership in ABI:**
  ...
  - [x] `abi-models` and `abi-model-runtime` foundations: Candle + safetensors, CPU and Metal first, CUDA
    ...
    recorded license acceptance. The current ABI crate and Abbey's exact-loaded-model v3
    route prove only the generated `abi-bigram-v1` fixture; Abbey has no production model
    architecture, weights, or broad prompt-inference route.
  - [x] Signed protocol-v3 model lifecycle: an edition-scoped owner-only startup
    document binds explicit publisher public keys, signed immutable manifests, an exact
    accepting principal and ledger, external storage, one device, and artifact bounds.
    Only a completely valid authority negotiates `read_models`, `download_models`, and
    `manage_models`. Exact download/load/unload actions use globally single-use schema-v7
    operation IDs, one concurrent lifecycle action per model revision, monotonic bounded
    progress, restart-to-failed recovery for ambiguous work, ABI resumable/hash-verified
    publication, ABI artifact/license re-verification, and no model/device fallback.
    Typed client methods send once and correlate status; a real default daemon with
    generated external scratch artifacts proves download verification, CPU load,
    loaded-with-no-inference-evidence status, unload, and durable terminal reopen. Unit
    fixtures also prove range download into empty storage, signature/permission refusal,
    license/revision/operation refusal, and generated CPU loader execution. No model bytes
    are committed. Production prompt inference, production Gemma, live HTTPS publisher evidence,
    cancellation, CLI/desktop, Windows, remote MCP, training, and workers remain open.
  - [x] Fixture-bounded exact-loaded-model inference: separately negotiated `infer_models`
    runs only an already-loaded immutable revision through ABI `LocalModelProvider` and
    `run_provider`, with domain-separated request/output digests, 32 KiB prompt/output
    bounds, 1..=256 output tokens, finite one-second/event/token/capture budgets, exact
    requested/executed-device and native-operation evidence, and no fallback, mixed
    execution, tools, substitution, route/default installation, retry, or downgrade. The
    existing exact-key active guard serializes lifecycle and inference while `Arc` avoids
    holding the loaded-map lock. The typed client sends once and recomputes correlation and
    digests. Real-daemon CPU-fixture proof covers unload/restart authority loss and verifies
    prompt/output are absent from audit/runtime.sqlite. This is synchronous, non-durable,
    owner-only Unix with no CLI/desktop/MCP/Windows/remote/protocol-v2/default route and no
    production architecture, weights, quality, performance, accelerator, or residency claim.
...
- [ ] **Phase 7 — Tauri 2 React/TypeScript desktop product:**
  - [x] **macOS Routes acceptance, 2026-09-06:** ordinary safe-edition Tauri
    window against a real owner-only scratch `abbeyd`, driven through native
    Accessibility by `desktop/scripts/prove-routes-macos.sh`. Exact 25/10/50-row
    tails, workspace digests, path/bearer canaries, authentication rejection,
    daemon-loss refusal with no stale rows/local fallback, empty-state rendering,
    and unchanged route-log hashes passed. Thirteen assertion/isolation tests,
    `ABBEY_DESKTOP_ROUTES_ACCEPTANCE=1 ./desktop/check.sh`, and root `./check.sh`
    passed. This closes only this macOS Routes slice; packaging/signing, other
    views, personal-edition GUI, and other platforms remain unproven. The
    `desktop-tauri-react` claim and Phase 7 remain Proposed/open.
  - **Slice 3 built 2026-08-27 (ledger row stays Proposed):** desktop Memory
    view over protocol-v3 `ReadMemory` search/metadata. `app_v3_grants` requests
    that grant only. Seeded scratch-`abbeyd` proof goes through the shipped
    desktop backend; payload/provenance/path/raw ids do not appear. Obsolete is
    not registered. In-process grants are empty and memory commands reject.
  ...
  - Tauri 2 + React + TypeScript + Bun/Vite as a client of `abbeyd`; no duplicated
    ...
    still fails closed. The macOS Routes-only window slice passed on 2026-09-06
    as recorded above; packaging/signing and submit/cancel remain unproven.
  - First-release views: Chat, Runs/trace, Tools/approvals, Memory/semantic spaces,
    ...
    Doctor/settings
    — REAL: Doctor/settings, Claims, Routes (in-process `ReadRoutes`), Runs &
    trace when `ReadRun` is granted, and Memory when v3 `ReadMemory` is granted.
    Availability is computed from live v2 capabilities plus `app_v3_grants`,
    not a hardcoded boolean. Submit, cancel, invoke, and obsolete are not
    registered. In-process never opens the memory store.
    — STILL PROPOSED: Chat, Tools, Models, Training, and Cluster render no data.
    Windowed live-`abbeyd` proof is limited to the safe macOS Routes slice above.
  - Security: strict CSP, no remote JS/eval, no generic execute-shell invoke, narrow
    ...
    `src`/`href`, remote dynamic `import()`, `eval(`, and `new Function(`; exactly ten
    enumerated read-only commands (`app_status`, `app_claims`, `app_routes`,
    `app_run_status`, `app_run_events`, `app_v3_grants`, `app_memory_search`,
    `app_memory_metadata`, `app_connection`, `app_bundle_identity`)
...
- [x] Config `backend` key (`cursor|grok|fm|abi|claude`); precedence env > config > resolvable cursor then fixed fallback order;
...
## Claude Code backend (`ABBEY_BACKEND=claude`)

- [x] Explicit selection plus last-in-order automatic fallback; candidate paths cannot fall through to cursor-agent
- [x] Isolated argv grammar, model-vocabulary clamp, and no cursor/fm/abi flag leakage
- [x] Claude-owned session mint/resume with an Abbey presence marker only
- [x] Docs/claims limit Current status to deterministic wiring; live account, catalog, plan, and quality remain unclaimed
