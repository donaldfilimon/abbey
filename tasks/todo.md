...
- [ ] **Phase 4 — Shared Abbey application core + durable `abbeyd` daemon:**
  ...
  - [ ] **Phase 4B.6 — cut `clear_chat` scope/all mutations over to canonical `runtime.sqlite` tombstones with the same owner-only journal, idempotent edition-scoped mirror removal, and crash/reopen proof; do not delete transcripts or memory and add no backend/title/run inference, protocol/UI/MCP, or Windows authority.**
