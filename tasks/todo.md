...
- [ ] **Phase 9 — MCP hosting without exposing the personal shell:**
  ...
  - Still Proposed / not implemented by slice 1: stateless Streamable HTTP; the MCP
    2026-07-28 stateless lifecycle (slice 1 implements the classic `initialize`
    handshake and negotiates 2025-06-18 / 2025-03-26 / 2024-11-05); `resources`,
    `prompts`, and `tasks` capabilities; concurrency limits (one frame is handled at a
    time); cooperative mid-flight cancellation (`notifications/cancelled` is accepted as
    a no-op — the timeout abandons a slow handler rather than signalling it)
  - Non-loopback: HTTPS, OAuth 2.1, PKCE, resource indicators, token audience binding,
    no token passthrough; rate limits — **none of this exists**; there is no network
    transport at all, so there is nothing yet to authenticate
  - `shell.exec` absent from remotely discoverable MCP capabilities — **done for stdio**
    and enforced structurally (`EffectClass` has one variant) plus asserted by
    `src/mcp_host/tests.rs::no_advertised_tool_is_execution_capable` and
    `tests/mcp_server.rs::the_advertised_registry_contains_no_execution_capable_tool`
  - Evidence bar: protocol conformance ✔, malformed-frame ✔, oversized-frame ✔,
    unknown-method ✔, timeout ✔, secret-redaction ✔ (21 unit + 7 process tests).
    Still open: hostile-origin, authn, token-audience, rate-limit, slow-consumer —
    all of these need the HTTP transport first
