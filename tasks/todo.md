...
Ledger snapshot after the 2026-08-08 Phase 4B.3 closure: **117 checked,
...
- [ ] **Phase 4 — Shared Abbey application core + durable `abbeyd` daemon:**
  ...
  - [ ] **Phase 4B.4 — idempotent legacy conversation metadata migration into
    runtime.sqlite on scratch copies, with retained backup and no transcript or
    semantic-memory import:** import only bounded canonical edition-scoped chat identity
    metadata after an exact owner-only content-addressed backup; never infer backend,
    title, transcript, memory, run, prompt, or provider content.
