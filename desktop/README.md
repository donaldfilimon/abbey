...
scaffold exposes read-only `Status`, `Claims`, and `ReadRoutes` app-core commands,
plus connection and bundle-identity metadata. Abbey's app
...
desktop/
  ...
  src/
    ...
    ipc/client.ts                  typed wrappers over the five commands
    ...
    views/DoctorView.tsx  ClaimsView.tsx  UnavailableView.tsx  ErrorCard.tsx
  src-tauri/
    ...
    src/commands.rs                the five commands
...
Desktop source inventory: **5 enumerated commands · 3 read capabilities · 3 available views · 7 unavailable views**.

- Invoke IDs: `app_status`, `app_claims`, `app_routes`, `app_connection`, `app_bundle_identity`
- Read capability IDs: `read_status`, `read_claims`, `read_routes`
- Available view IDs: `doctor` → `read_status`, `claims` → `read_claims`, `routes` → `read_routes`
- Unavailable view IDs: `chat`, `runs`, `tools`, `memory`, `models`, `training`, `cluster`
<!-- END abbey-generated:desktop-capability-summary -->

## The complete Tauri command surface

Five commands, all read-only, enumerated in `generate_handler!`:
...
There is no `exec`, no `run`, no command-name parameter that resolves to a
subprocess, and no plugin that would provide one.
