...
desktop/
  ...
  src/
    ...
    ipc/client.ts                  typed wrappers over the ten commands
  ...
  src-tauri/
    ...
    src/commands.rs                the ten commands
...
Desktop source inventory: **12 enumerated commands · 7 read capabilities · 6 available views · 4 unavailable views**.
...
- Read capability IDs: `read_status`, `read_claims`, `read_routes`, `read_run`, `read_run_events`, `read_memory`, `read_models`
...
Ten commands, all read-only, enumerated in `generate_handler!`:
...
| `app_v3_grants` | `V3CapabilitySet` | negotiate `ReadMemory` only |
