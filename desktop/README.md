...
desktop/
  ...
  src/
    ...
    ipc/client.ts                  typed wrappers over the four commands
  ...
  src-tauri/
    ...
    src/commands.rs                the four commands
...
Four commands, all read-only, enumerated in `generate_handler!`:
...
**Routing.** If a daemon bearer is configured for the active edition, every read
...
Training, Cluster, Routes. These render **no data at all**. Each states the
