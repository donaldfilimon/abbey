...
def normalize_manifest(raw: object) -> dict[str, Any]:
    ...
    # Schema 2 added the terminal lifecycle statuses to the `status` value set.
    # A schema-1 reader has a five-way allowlist and raises on the new values,
    # which is why the vocabulary change is a schema change and not a silent one.
    if raw.get("schema_version") != 2:
        raise ValueError("claims manifest schema_version must be 2")
    ...
    statuses = {
        ...
        "blocked",
        "out_of_scope",
        # Terminal lifecycle states (constitution decision 68). Empty in a
        # healthy registry, but the allowlist must admit them BEFORE a claim
        # needs one -- otherwise the first real failure breaks the sync tool.
        "failed",
        "revoked",
        "superseded",
        "expired",
...
def status_counts(manifest: dict[str, Any]) -> str:
    positional = ("current", "partial", "proposed", "blocked", "out_of_scope")
    lifecycle = ("failed", "revoked", "superseded", "expired")
    order = positional + lifecycle
    labels = {
        "current": "Current",
        "partial": "Partial",
        "proposed": "Proposed",
    ...
    # Positional states always report, including a zero, because a zero there
    # is itself a finding. Lifecycle states report only when occupied, so a
    # healthy ledger header does not carry four permanent zeroes.
    shown = [s for s in positional] + [s for s in lifecycle if counts[s]]
    return " · ".join(f"{counts[status]} {labels[status]}" for status in shown)
...
def render_region(
    ...
    lines.append(
        ...
        f"([full evidence]({link})). Source: {source}; "
        f"schema {manifest['schema_version']}; digest `{digest}`."
    )
    lines.append(
