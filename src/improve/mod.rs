...
impl RunReport {
    fn new(correlation: &str, focus: &WorkFocus, opts: &ImproveOpts) -> Self {
        Self {
    ...
    fn note_gate(&mut self, round: usize, g: &GateReport) {
    ...
    fn note_diagnose(&mut self, round: usize, results: &[LaneResult]) {
        let names: Vec<_> = results.iter().map(|r| r.name.as_str()).collect();
        self.lines
            .push(format!("- round {round} diagnose: {}", names.join(", ")));
    }

    fn note_implement(&mut self, round: usize, r: &LaneResult) {
        self.lines.push(format!(
...
fn report_path(state_dir: &Path, correlation: &str) -> PathBuf {
    state_dir.join("improve").join(format!("{correlation}.md"))
}

fn write_report(state: &AbbeyState, report: &RunReport) -> Result<()> {
...
fn latest_report_path(state_dir: &Path) -> Option<PathBuf> {
