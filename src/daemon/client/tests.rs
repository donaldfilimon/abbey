...
fn debug_and_errors_do_not_reveal_bearer() {
    ...
    let (config, thread, root) = fake_v3_server(|request| {
        encoded_v3_response(V3ResponseEnvelope::error(
            request.request_id,
            V3ErrorCode::Unauthorized,
            format!("peer echoed {TEST_BEARER}"),
        ))
    });
    ...
    let error = DaemonClient::new(config)
        ...
        .unwrap_err();
    assert!(matches!(&error, ClientError::DaemonV3 { .. }));
    assert!(!format!("{error:?}").contains(TEST_BEARER));
    assert!(!error.to_string().contains(TEST_BEARER));
    thread.join().unwrap();
    std::fs::remove_dir_all(root).unwrap();
...
fn fake_v3_server<F>(responder: F) -> (DaemonConfig, thread::JoinHandle<()>, PathBuf)
where
    F: FnOnce(V3RequestEnvelope) -> Vec<u8> + Send + 'static,
{
    let root = scratch_dir("fake-v3");
    let config = test_config(root.join("abbeyd.sock"));
    let listener = UnixListener::bind(&config.socket_path).unwrap();
    let thread = thread::spawn(move || {
        let (mut stream, _) = listener.accept().unwrap();
...
fn read_v3_request(stream: &mut UnixStream) -> V3RequestEnvelope {
    let mut prefix = [0_u8; 4];
    stream.read_exact(&mut prefix).unwrap();
    let mut bytes = vec![0_u8; u32::from_be_bytes(prefix) as usize];
    stream.read_exact(&mut bytes).unwrap();
...
fn encoded_v3_response(response: V3ResponseEnvelope) -> Vec<u8> {
    framed(&serde_json::to_vec(&response).unwrap())
}
