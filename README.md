...
supports exact stable-ID claim lookup from the canonical registry and additionally
negotiates `read_models` when an ABI-local provider was bound at daemon startup;
`ListModels` returns bounded fixed-watermark inventory derived from that same provider
object. Missing or non-exact claim IDs return `not_found`, and fuzzy claim search remains
unavailable. Every other v3 grant remains denied. Create a private bearer file and start
it explicitly:
