...
environment, workspace, tool, or shell. Create a private bearer file and start it
explicitly:
...
import transcripts or memory. Separately, normal Unix identity saves commit opaque
edition-scoped metadata before an owner-only recovery journal projects the existing
compatibility mirrors; reads and clears remain mirror-based. That coordinator adds no
daemon command or protocol. This surface is not the Proposed provider-neutral
Abbey-owned agent/tool runtime.
...
src/
  ...
  app_core/       protocol-v1 reads + protocol-v2 bounded run contracts
  ...
  state/          canonical-first identity-save coordinator + private mirror journal
