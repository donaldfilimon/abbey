...
environment, workspace, tool, or shell. Protocol v3 uses a separate strict envelope and
negotiates only `read_models` when an ABI-local provider was bound at daemon startup;
`ListModels` returns bounded fixed-watermark inventory derived from that same provider
object. Every other v3 grant remains denied. Create a private bearer file and start it
explicitly:
