...
supports bounded fixed-watermark inventory from the same structurally read-only safe tool
registry used by MCP, plus exact stable-ID claim lookup from the canonical registry. It
additionally negotiates `read_models` when an ABI-local provider was bound at daemon
startup; `ListModels` derives inventory from that same provider object. Missing or
...
policy validation. Schema v5 contains a dormant digest-bound approval-state ledger, but
approval and cancellation grants, mutating tools, and every other unserved v3 grant remain
denied. Create a private
