...
`fm`, `abi` (doctor shows the automatic choice), and `ABBEY_BACKEND` still selects
one explicitly. No single backend is a hard requirement.
...
approval decisions, cancellation, execution after approval, and every other unserved v3
grant remain denied. Create a private
...
- The preferred default backend is **cursor-agent**, but it is never required: `fm`,
  `abi`, and `grok` serve configured routes without it, and the unchosen default falls
