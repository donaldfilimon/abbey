...
- Shared application library plus `abbeyd`: exact versioned Status/Claims events
  ...
  agent/tool runtime, durable job manager, MCP client/host, or Windows named-pipe proof.
- Durable lifecycle library foundation: `abbey::runtime` defines a separate
  ...
  deterministic manager over an injected executor. It is not routed through protocol
  v1 and does not grant the shipped daemon model/tool execution authority. See
