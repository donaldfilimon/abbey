...
| `lib` / `app_core` | Private implementation graph plus public protocol-v1 reads, protocol-v2 bounded-run contracts/policy, and separate deny-by-default protocol-v3 contracts; safe read-only tool inventory and exact stable-claim reads are available while `read_models` inventory is conditional on startup-bound ABI authority |
...
| `daemon` / `bin/abbeyd` | Authenticated 64 KiB Unix transport: exact v1 reads, v2 startup-bound ABI `ModelProvider` run control, and a separate v3 envelope plus typed negotiated client for canonical safe-tool inventory and audited read-only invocation, exact claim lookup, and conditional ABI-local model inventory; no approval, cancellation, mutating/personal shell, or memory authority; non-Unix fails closed |
