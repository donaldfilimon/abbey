...
| `daemon` / `bin/abbeyd` | Authenticated 64 KiB Unix transport: exact v1 reads, v2 startup-bound ABI `ModelProvider` run control, and a separate v3 envelope with ABI-local model inventory only; no arbitrary tool/shell/memory authority; non-Unix fails closed |
