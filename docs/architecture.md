...
| `lib` / `app_core` | Private implementation graph plus public protocol-v1 reads, protocol-v2 bounded-run contracts/policy, and separate deny-by-default protocol-v3 contracts; only `read_models` inventory can currently be granted |
...
| `daemon` / `bin/abbeyd` | Authenticated 64 KiB Unix transport: exact v1 reads, v2 startup-bound ABI `ModelProvider` run control, and a separate v3 envelope plus typed negotiated client/CLI for ABI-local model inventory only; no arbitrary tool/shell/memory authority; non-Unix fails closed |
