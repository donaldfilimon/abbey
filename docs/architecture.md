...
| `daemon` / `bin/abbeyd` | Authenticated 64 KiB Unix transport: exact v1 reads, v2 startup-bound ABI `ModelProvider` run control, and a separate v3 envelope plus typed negotiated client for canonical safe-tool inventory, exact claim lookup, and conditional ABI-local model inventory; no tool invocation, approval, shell, or memory authority; non-Unix fails closed |
