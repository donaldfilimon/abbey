...
- Durable lifecycle foundation: `abbey::runtime` defines a separate
  ...
  deterministic manager over a fixed delegated executor. Protocol v2 exposes only that
  bounded route; provider-neutral model/tool ownership, daemon memory, live subscriptions,
  arbitrary commands, and Windows IPC remain absent. See [runtime architecture](runtime.md).
