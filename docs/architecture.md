...
| `mcp_host` | Abbey's own read-only stdio MCP **server** (`abbey mcp serve`): JSON-RPC 2.0 framing, version negotiation, `tools` capability, safe registry, enforced limits, outbound redaction. No HTTP/TLS/OAuth |
...
- Durable lifecycle foundation: `abbey::runtime` defines a separate
  ...
  timestamps transactionally. Normal Unix identity saves now commit opaque edition/scope/
  alias/mutation metadata to schema-v3 SQLite before an owner-only locked coordinator
  projects the exact compatibility mirrors; verified recovery discards uncommitted plans,
  completes committed plans once, and refuses divergent destinations. Reads and clears
  remain legacy-mirror behavior, and this coordinator adds no command or presentation
