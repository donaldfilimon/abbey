...
| `lib` / `app_core` | Private implementation graph plus public protocol-v1 reads, protocol-v2 bounded-run contracts/policy, and separate deny-by-default protocol-v3 contracts; safe tool inventory, audited read-only invocation, one typed digest-bound pending mutation, and exact stable-claim reads are available while `read_models` inventory is conditional on startup-bound ABI authority |
...
| `daemon` / `bin/abbeyd` | Authenticated 64 KiB Unix transport: exact v1 reads, v2 startup-bound ABI `ModelProvider` run control, and a separate v3 envelope plus typed negotiated client for canonical read-only tools, one default-only local mutation that can only create a digest-bound pending approval, exact claim lookup, and conditional ABI-local model inventory; no approval decision, cancellation, approved-call execution, personal shell, or daemon-memory execution; non-Unix fails closed |
