...
│  agent — cursor · grok · fm · abi (`abi complete`) · Claude Code executors │
└─────────────────────────────────────────────────────────┘
```
...
| `lib` / `app_core` | Private implementation graph plus public protocol-v1 reads, protocol-v2 bounded-run contracts/policy, and separate deny-by-default protocol-v3 contracts; safe tool inventory, audited read-only invocation, one typed digest-bound pending mutation, summary-only memory reads, and exact stable-claim reads are available while `read_models` inventory is conditional on startup-bound ABI authority |
...
| `daemon` / `bin/abbeyd` | Authenticated 64 KiB Unix transport: exact v1 reads, v2 startup-bound ABI `ModelProvider` run control, and a separate v3 envelope plus typed negotiated client for canonical read-only tools, one default-only exact-approved local mutation, summary-only memory inventory/search/metadata with opaque IDs, exact claim lookup, and conditional ABI-local model inventory; no personal shell, raw memory projection, semantic-vector search, or remote authority; non-Unix fails closed |
...
- Abi-backend execution via `ABBEY_BACKEND=abi` (`abi complete`): own argv grammar with
  ...
  Persistent default via config `backend = "cursor"|"grok"|"fm"|"abi"|"claude"`
  (`ABBEY_BACKEND` env wins); in-TUI Ctrl-B cycles resolvable backends
- Claude Code execution via `ABBEY_BACKEND=claude`: isolated argv grammar, bounded
  model-alias mapping, and native Claude session mint/resume selected by an Abbey
  presence marker. Source/unit-tested wiring only; no live account, catalog, plan, or
  response-quality claim.
...
- ABI and WDBX are public and immutably pinned, so neither checkout requires a
  repository secret. An online, enabled macOS ARM64 self-hosted path exists for
  trusted events, and an `ubuntu-latest` path exists for foreign pull requests.
  Neither is hosted execution evidence until an exact-head job completes
  successfully. Linux ARM64 is not provisioned, and Windows runtime proof
  remains open.
