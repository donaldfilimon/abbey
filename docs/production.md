...
can only validate an opaque memory-record ID and persist a digest-bound pending approval;
it stores no raw call, performs no mutation, and is absent from personal and MCP registries.
No client-selected tool registry, arbitrary shell, daemon memory execution, automation,
approval decision, cancellation, approved-call execution, or live event subscription is
exposed. Windows named-pipe and
