...
memory, models, route/run data, and finalized migration backups. Reads remain
compatibility-mirror resolution until their own canonical selection proof. Scratch
default/WDBX process evidence establishes only Unix-local identity mutation and recovery;
it does not establish a protocol, UI, MCP, Windows, backend/title/run-inference, or
general user-data deletion capability.
