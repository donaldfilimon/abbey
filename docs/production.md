...
These remain the current human CLI commands; protocol-v2 run control is presently a typed
client/server contract rather than a CLI/TUI lifecycle surface. Human output is concise;
`--json` is the typed `AppEvent`. Connection,
