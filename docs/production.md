...
Normal Unix conversation-identity saves use a separate canonical-first coordinator. A
schema-v3 immediate transaction stores only domain-separated edition/scope/alias/
scope-set/mutation digests, the deterministic UUIDv8 identity, revision, and timestamp.
Only after that commit does the owner-only, `fs4`-locked
`<state>/daemon/conversation-mirror-journal/` project the configured `chat-id`, export,
direct `by-cwd`, and history mirrors. The journal directory is `0700`; `lock` and the
transient atomic `pending.json` are `0600`. The pending plan may briefly contain raw
mirror material needed for recovery, but diagnostics redact it. Reopen discards a plan
without an exact canonical mutation marker, completes an exact committed plan once, and
refuses divergent or aliased destinations rather than overwriting them. Reads and
`clear_chat` remain mirror-only behavior in this slice. Scratch default/WDBX process
proof does not establish a protocol, UI, MCP, Windows, transcript, memory, backend/title,
or run-inference capability.
