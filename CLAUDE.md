...
| `runtime/` (`manager.rs`, `store.rs`, `store/identity.rs`, `identity.rs`, `supervisor.rs`, `executor.rs`, `delegated.rs`, `migrations.rs`) | durable run lifecycle, opaque schema-v3 conversation identity metadata, bounded process-group supervision, and fixed ABI-local/Foundation Models adapters; no provider-neutral model or tool runtime |
...
| `agent/` (`mod.rs`, `argv.rs`) / `models.rs` / `gitops.rs` / `state.rs` / `state/conversation.rs` / `config.rs` | executor invocation (cursor · grok · on-device `fm` · `abi complete`; `argv.rs` holds the per-backend argv grammars + prompt/argv clamping), model aliases, local git helpers, per-cwd state, canonical-first identity saves with a private compatibility-mirror journal, config loading |
...
- Conversation mirrors are projections: commit the opaque canonical save first, then
  recover only the exactly marked plan through the owner-only locked journal. Reads and
  clear remain legacy-mirror behavior until their own evidence slices land.
