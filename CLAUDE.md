...
| `runtime/` (`manager.rs`, `store.rs`, `supervisor.rs`, `executor.rs`, `delegated.rs`, `migrations.rs`) | durable run lifecycle, bounded process-group supervision, and fixed ABI-local/Foundation Models adapters; no provider-neutral model or tool runtime |
